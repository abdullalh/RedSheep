...
Original file is located at
    https://colab.research.google.com/drive/1sr9xtdoFAILfdA9ev8koTKuz5HsFiXkm
"""
...
class Room:
    def __init__(self, room_number: int, room_type: str, amenities: List[str], price_per_night: float):
        try:
            ...
            if not isinstance(price_per_night, float) and not isinstance(price_per_night, int):
                raise TypeError("Price per night must be a number")
            ...
            self._price_per_night = float(price_per_night)  # Price per night for the room
    ...
    def set_price_per_night(self, price_per_night: float):
        try:
            ...
            if price_per_night <= 0:
                raise ValueError("Price per night must be positive")
            self._price_per_night = float(price_per_night)  # Set the price per night
        except (TypeError, ValueError) as e:
            raise e
    ...
    def set_available(self, is_available: bool):
        try:
            if not isinstance(is_available, bool):
                raise TypeError("Availability status must be a boolean")
...
class StandardRoom(Room):
    def __init__(self, room_number: int, amenities: List[str], price_per_night: float, bed_size: str, has_tv: bool, has_mini_fridge: bool):
        try:
            ...
            if not isinstance(bed_size, str):
                raise TypeError("Bed size must be a string")
            if not isinstance(has_tv, bool):
                raise TypeError("TV availability must be a boolean")
            if not isinstance(has_mini_fridge, bool):
                raise TypeError("Mini fridge availability must be a boolean")
    ...
    def __init__(self, room_number: int, amenities: List[str], price_per_night: float, bed_size: str, has_balcony: bool, has_jacuzzi: bool):
        try:
            ...
            if not isinstance(has_balcony, bool):
                raise TypeError("Balcony availability must be a boolean")
            if not isinstance(has_jacuzzi, bool):
                raise TypeError("Jacuzzi availability must be a boolean")
...
class SuiteRoom(Room):
    def __init__(self, room_number: int, amenities: List[str], price_per_night: float, has_living_area: bool, has_kitchen: bool, has_private_pool: bool):
        try:
            ...
            if not isinstance(has_living_area, bool):
                raise TypeError("Living area availability must be a boolean")
            if not isinstance(has_kitchen, bool):
                raise TypeError("Kitchen availability must be a boolean")
            if not isinstance(has_private_pool, bool):
                raise TypeError("Private pool availability must be a boolean")
...
class Guest:
    def __init__(self, name: str, contact_info: str, loyalty_status: str):
        try:
            ...
            if not isinstance(loyalty_status, str):
                raise TypeError("Loyalty status must be a string")
    ...
    def set_loyalty_points(self, loyalty_points: int):
        try:
            if not isinstance(loyalty_points, int):
...
class Booking:
    def __init__(self, booking_id: int, room: Room, guest: Guest, check_in_date: str, check_out_date: str):
        try:
            ...
            try:
                ...
                raise ValueError(f"Invalid date format or date logic: {str(e)}")
    ...
    def set_check_in_date(self, check_in_date: str):
        try:
            ...
            self._total_price = self.calculate_total_price()  # Recalculate total price
        except (TypeError, ValueError) as e:
            raise e
    ...
    def set_check_out_date(self, check_out_date: str):
        try:
            ...
            try:
                out_date = datetime.strptime(check_out_date, '%Y-%m-%d')
                in_date = datetime.strptime(self._check_in_date, '%Y-%m-%d')
                if out_date <= in_date:
                    raise ValueError("Check-out date must be after check-in date")
            except ValueError as e:
                raise ValueError(f"Invalid date format or date logic: {str(e)}")
...
class Payment:
    def __init__(self, payment_id: int, booking: Booking, amount: float, payment_method: str):
        try:
            if not isinstance(payment_id, int):
                raise TypeError("Payment ID must be an integer")
            if not isinstance(booking, Booking):
                raise TypeError("Booking must be a Booking object")
            ...
            self._amount = float(amount)  # Amount to be paid
    ...
    def set_amount(self, amount: float):
        try:
            ...
            if amount <= 0:
                raise ValueError("Amount must be positive")
            self._amount = float(amount)  # Set the amount to be paid
        except (TypeError, ValueError) as e:
            raise e
    ...
    def process_payment(self) -> bool:
        try:
            ...
            # In a real application, this would include calls to payment gateways
    ...
    def redeem_reward(self, reward: str) -> bool:
        try:
            if not isinstance(reward, str):
                raise TypeError("Reward must be a string")
...
class GuestServiceRequest:
    ...
    def update_status(self, status: str):
        try:
            self.set_status(status)  # Update the status of the request
        except (TypeError, ValueError) as e:
            raise e
    ...
    def __init__(self, feedback_id: int, guest: Guest, rating: int, comments: str):
        try:
            if not isinstance(feedback_id, int):
                raise TypeError("Feedback ID must be an integer")
            if not isinstance(guest, Guest):
                raise TypeError("Guest must be a Guest object")
